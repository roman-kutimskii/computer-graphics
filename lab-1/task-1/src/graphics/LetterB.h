...
public:
    LetterB(const sf::Vector2f &position, const sf::Color &color): Letter(position) {
...
private:
    void createShapes(const sf::Vector2f &position, const sf::Color &color) {
        ...
        auto circle1 = std::make_unique<sf::CircleShape>(25);
        circle1->setPosition(position + sf::Vector2f(15, 0));
        circle1->setFillColor(color);
        m_shapes.push_back(std::move(circle1));

        auto circle2 = std::make_unique<sf::CircleShape>(15);
        circle2->setPosition(position + sf::Vector2f(25, 10));
        circle2->setFillColor(sf::Color::White);
        m_shapes.push_back(std::move(circle2));

        auto circle3 = std::make_unique<sf::CircleShape>(30);
        circle3->setPosition(position + sf::Vector2f(15, 30));
        circle3->setFillColor(color);
        m_shapes.push_back(std::move(circle3));

        auto circle4 = std::make_unique<sf::CircleShape>(20);
        circle4->setPosition(position + sf::Vector2f(25, 40));
        circle4->setFillColor(sf::Color::White);
        m_shapes.push_back(std::move(circle4));
    }
};
